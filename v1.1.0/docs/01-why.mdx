...
When creating interactive learning resources, we need a better abstraction for
developing editing experiences that are not primarily dealing with rich text.
...
Providing a great editing experience for such use cases often requires us to
have a tight control over the user experience. Trying to achieve that with a
rich text library like Slate is hard, because:
...
These reasons lead us to build Edtr.io as an abstraction layer on top of that: A
composable plugin system representing a tree of content elements where rich text
is only contained in the leaf nodes.
